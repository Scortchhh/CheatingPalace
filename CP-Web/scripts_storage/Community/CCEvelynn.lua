...
function Evelynn:__init()
    ...
    self.ScriptVersion = "        **CCEvelynn Version: 0.4 (BETA)**"
    ...
    if self.ComboQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        ...
        if target then
            if not myHero.BuffData:GetBuff("EvelynnQ2").Valid then
                local PredPos, Target = Prediction:GetCastPos(myHero.Position, 777, self.QSpeed, self.QRadius, self.QDelay, 1)
                if PredPos then
...
function Evelynn:Laneclear()
    if self.ClearQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        ...
        local target = Orbwalker:GetTarget("LanceClear", self.QRange)
