...
function Yone:__init() 
    ...
    self.ScriptVersion = "         dYone Ver: 1.01 CREDITS Derang3d" 
...
function Yone:Combo()
    ...
    if Engine:SpellReady("HK_SPELL1") then
		...
		if QBuff.Valid == true then
			if self.HarassQ.Value == 1 and self.Comboq3.Value == 1 then
				...
				local CastPos, Target 		= Prediction:GetCastPos(StartPos, self.q3Range - 100, 1500, 0, 0.25, 0)
				if CastPos ~= nil and Target ~= nil then
					...
					if Distance < myHero.AttackRange + myHero.CharData.BoundingRadius then
						if Orbwalker.Attack == 0 then
							if GetDist(StartPos, CastPos) < self.q3Range then
    ...
    if Engine:SpellReady("HK_SPELL1") then
        if self.ComboQ.Value == 1 then
            local QBuff = myHero.BuffData:GetBuff("yoneq3ready")
            if QBuff.Valid == false then
                local Target = Orbwalker:GetTarget("Combo", self.Q1Range - 40)
                if Target then
