...
function Orbwalker:CanWaveClear(Target)
	...
	local damage = (PlayerDamage * TargetArmorMod) + self.ExtraDamage - 5

	if Target.Health - (damage + IncomingDamage) > 0 and Target.Health - (damage + IncomingDamage) <= 80 then
		return false
	end

	if Target.Health - (damage + IncomingDamage) <= 0 then
		return true
	end

	if Target.Health - damage >= damage + 70 then
		return true
	end
		...
		for I, Minion in ipairs(Minions) do
			if Minion.Team ~= myHero.Team then
				if self:GetDistance(myHero.Position, Minion.Position) < (Range + Minion.CharData.BoundingRadius) then
					...
					if self:CanWaveClear(Minion) then
...
function Orbwalker:CanAttack()---------------------------------TODO Champions with ammo
	if self:IsChanneling() == false then
		...
		if myHero.MaxAmmo > 0 and myHero.Ammo < 1 and myHero.ChampionName ~= "Corki" and myHero.ChampionName ~= "Xayah" then
			return false
		end
