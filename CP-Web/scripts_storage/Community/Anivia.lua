...
function Anivia:GetQCastPos(CastPos)
...
function Anivia:Combo() 
    ...
    if target then
        local CastPos = Prediction:GetCastPos(myHero.Position, 900, math.huge, 100, 0.25, 0)
        ...
        if not qBuff.Valid and self.UseQCombo.Value == 1 then
            if Engine:SpellReady("HK_SPELL1") then
                if Anivia:GetDistance(myHero.Position, target.Position) <= 1000 then
                    local castPos = Prediction:GetCastPos(myHero.Position, 1000, 800, 130, 0.25, 0)
                    if castPos ~= nil then
        ...
        if isStunned.Valid then
            if Anivia:GetDistance(myHero.Position, target.Position) <= 600 and Engine:SpellReady("HK_SPELL3") and self.UseECombo.Value == 1 then
...
function Anivia:Harass() 
    ...
    if target then
        ...
        local isStunned = target.BuffData:GetBuff("aniviaiced")
        if isStunned.Valid then
            if Anivia:GetDistance(myHero.Position, target.Position) <= 600 and Engine:SpellReady("HK_SPELL3") and self.UseEHarass.Value == 1 then
                Engine:CastSpell("HK_SPELL3", target.Position, 1)
            end
        end
    end
