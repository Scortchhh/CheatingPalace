...
function Kassadin:CastE(target)
	local PredPos = Prediction:GetPredPos(myHero.Position, target, math.huge, 0.2)
...
function Kassadin:CastR(target, gapclose)
	if gapclose == false then
		local PredPos = Prediction:GetPredPos(myHero.Position, target, math.huge, 0.25)
...
function Kassadin:Harass()
	if Engine:SpellReady("HK_SPELL2") then
		if Orbwalker:GetTarget("Harass", GetAttackRange()) == Orbwalker:GetTarget("Combo", 1500) then
			if Orbwalker:CanAttack() == false and Orbwalker.WindingUp == 0 then
				Engine:CastSpell("HK_SPELL2", Vector3.new() ,1)
				return
			end
