...
function Kaisa:QDmg(Target)
    ...
    if myHero.BuffData:GetBuff("KaisaQEvolved").Valid then
        QDmg = Kaisa:GetDmg(107 + (61 * myHero:GetSpellSlot(0).Level) + (myHero.BonusAttack * 1.5) + (myHero.AbilityPower * 0.9375), true, Target)
    end
        ...
        if target2 ~= nil then
            if GetDist(myHero.Position, target2.Position) > self:getAttackRange() + 20 then
                if self:WDmg(target2) then
                    local PredPos, target = Prediction:GetCastPos(StartPos, self.WRange, self.WSpeed, 200, self.WDelay, 1)
                    if PredPos ~= nil then
...
function Kaisa:Harass()
    ...
    if self.HarassW.Value == 1 and Engine:SpellReady("HK_SPELL2") then
        ...
        if target2 ~= nil then print(1)
            if GetDist(myHero.Position, target2.Position) > self:getAttackRange() + 20 then print(2)
                local PredPos, target = Prediction:GetCastPos(StartPos, self.WRange, self.WSpeed, 200, self.WDelay, 1) print(3)
                if PredPos ~= nil then print(4)
                    if self.SlowWHarass.Value == 1 then print(5)
                        if target2.AIData.IsDashing == true or target2.BuffData:HasBuffOfType(BuffType.Stun) == true or target2.BuffData:HasBuffOfType(BuffType.Slow) == true or target2.BuffData:HasBuffOfType(BuffType.Snare) == true then print(6)
                            local mark = target2.BuffData:GetBuff("kaisapassivemarker")
                            local WStacks = 3
                            if myHero.BuffData:GetBuff("KaisaWEvolved").Valid then
                                WStacks = 2
                    ...
                    if self.SlowWHarass.Value == 0 then print(7)
                        local mark = target2.BuffData:GetBuff("kaisapassivemarker")
                        local WStacks = 3
                        if myHero.BuffData:GetBuff("KaisaWEvolved").Valid then
                            WStacks = 2
                        ...
                        if self.StackWHarass.Value == 0 then print(8)
                            Engine:CastSpell("HK_SPELL2", PredPos, 1)
                        end
                    end
                end
...
function Kaisa:EattackDisabler()
    if myHero.BuffData:GetBuff("KaisaE").Valid then
        Orbwalker:IssueMove(Vector3.new())
    end
end
