    ...
    if self.ComboKSE.Value == 1 and Engine:SpellReady("HK_SPELL3") and not Engine:SpellReady("HK_SPELL1") then
        ...
        if target then
            if self:EDmg(target) > target.Health then
                local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.ERange, self.ESpeed, self.ERadius, self.EDelay, 0)
                if PredPos then
    ...
    if self.ComboKSQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        ...
        if target then
            if self:MaxQDmg(target) > target.Health then
                ...
                local CastPos 	= Prediction:GetCastPos(StartPos, self.QMaxRange , self.QSpeed, self.QRadius, self.QDelay, 0)
                if CastPos ~= nil then
        ...
        if target then
            ...
            if myHero.Health <= myHero.MaxHealth / 100 * self.SingleRHP.Value then
                if self:KillableR(target) then
                    local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.RRange, self.RSpeed, self.RRadius, self.RDelay, 0)
                    if PredPos then
