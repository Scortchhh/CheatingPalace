...
function Ryze:Combo()
    if self.ComboAA.Value > self:GetLevel() then
        if self.ComboQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
            DelayAction(function()
                ...
                if Target ~= nil then
                    if GetDist(myHero.Position, Target.Position) < self:getAttackRange() and Orbwalker:CanAttack() == false and Orbwalker.WindingUp == 0 then
                        local PredPos, Target = Prediction:GetCastPos(StartPos, self.QRange, self.QSpeed, 55, self.QDelay, 1)
                        if PredPos ~= nil then
        ...
        if self.ComboE.Value == 1 and Engine:SpellReady("HK_SPELL3") and not Engine:SpellReady("HK_SPELL1") then
            DelayAction(function()
                local target = Orbwalker:GetTarget("Combo", self.ERange)
                if target ~= nil then
                    if GetDist(myHero.Position, target.Position) < self:getAttackRange() and Orbwalker:CanAttack() == false and Orbwalker.WindingUp == 0 then
        ...
        if self.ComboW.Value == 1 and Engine:SpellReady("HK_SPELL2") and not Engine:SpellReady("HK_SPELL1") then
            DelayAction(function()
...
function Ryze:Harass()
    if self.ComboAA.Value > self:GetLevel() then
        ...
        if self.HarassW.Value == 1 and Engine:SpellReady("HK_SPELL2") and not Engine:SpellReady("HK_SPELL1") then
        ...
        if self.HarassE.Value == 1 and Engine:SpellReady("HK_SPELL3") and not Engine:SpellReady("HK_SPELL1") then
