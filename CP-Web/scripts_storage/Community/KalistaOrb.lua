...
function Orbwalker:GetTarget(OrbMode, Range)
    ...
    if OrbMode == "Combo" then
        ...
        if myHero.ChampionName == "Kalista" and currentTarget == nil then
            local Minions = self:SortToNearest(ObjectManager.MinionList)
...
function Orbwalker:GetWindup()
    ...
    return math.min((AttackDelay * Multiplicator), AttackDelay)
end
...
function Orbwalker:CanAAReset()
    local AAResets = {
        ...
        "reksaiq", "itemtitanichydracleavebuff", "vaynetumblebonus"
    }
    for i = 1, #AAResets do
