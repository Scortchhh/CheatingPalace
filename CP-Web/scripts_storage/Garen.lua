...
function Garen:__init() -- initialize garen script
    ...
    self.ScriptVersion = "         dGaren Ver: 1.02 CREDITS Derang3d" -- tells what version of the script it is.
...
function Garen:Combo()
    ...
    if GetDist(myHero.Position, target.Position) <=1000 then
        if Engine:SpellReady('HK_SPELL1') and self.ComboQ.Value == 1 and Orbwalker.ResetReady == 1 then
	...
	if self.ComboE.Value == 1 and Engine:SpellReady("HK_SPELL3") then
		...
		if buff1.Count_Alt == 0 and buff2.Count_Alt == 0 and Target ~= nil then
...
function Garen:Laneclear()
    ...
    if self.ClearQ.Value == 1 and Engine:SpellReady("HK_SPELL1") and Orbwalker.ResetReady == 1 and buff1.Count_Alt == 0 and buff2.Count_Alt == 0  then
    ...
    if self.ClearE.Value == 1 and Engine:SpellReady("HK_SPELL3") and buff1.Count_Alt == 0 and buff2.Count_Alt == 0 then
        local MinionList = ObjectManager.MinionList
        for i, Minion in pairs(MinionList) do
            if Minion.Team ~= myHero.Team and Minion.IsDead == false and Minion.MaxHealth > 10 and MinionsInRange(myHero.Position, 325) >= 2  then
