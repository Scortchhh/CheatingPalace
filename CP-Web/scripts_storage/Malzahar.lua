...
function Malzahar:Combo()
    ...
    if self.ComboQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        local CastPos = Prediction:GetCastPos(myHero.Position, self.QRange, self.QSpeed, self.QWidth, self.QDelay, 0, true, self.QHitChance, 0) -- positionHero, range of spell - 50, speed of missle, width, delay, chanel time(most 0.25), 0 or 1 does not need minion check 0
        if CastPos ~= nil then
    ...
    if self.ComboE.Value == 1 and Engine:SpellReady("HK_SPELL3") then
        local target = Orbwalker:GetTarget("Combo", self.ERange, self.EDelay, 0.25) -- .25 vs 0 makes no difference
        if target ~= nil then
            return Engine:CastSpell("HK_SPELL3", target.Position, 1) -- 1 for champs 0 for minions
        end
    end

    if self.ComboW.Value == 1 and Engine:SpellReady("HK_SPELL2") then
        local CastPos = Prediction:GetCastPos(myHero.Position, self.WRange, self.WSpeed, self.WWidth, self.WDelay, 0, true, self.WHitChance, 0) -- positionHero, range of spell - 50, speed of missle, width, delay, chanel time(most 0.25), 0 or 1 does not need minion check 0
        if CastPos ~= nil then
            if GetDist(myHero.Position, CastPos) <= self.ERange then
...
function Malzahar:Ultimate()
    ...
    if self.ComboR.Value == 1 and Engine:SpellReady("HK_SPELL4") and (GameClock.Time - myHero:GetSpellSlot(0).Cooldown) < -0.25 and (GameClock.Time - myHero:GetSpellSlot(1).Cooldown) < -0.25 and (GameClock.Time - myHero:GetSpellSlot(2).Cooldown) < -0.25 then -- inside if you need ==, checking if it is same value, = is to make it that value
        local CastPos = Prediction:GetCastPos(myHero.Position, self.RRange, self.RSpeed, self.RWidth, self.RDelay, 0, true, self.RHitChance, 0) -- positionHero, range of spell - 50, speed of missle, width, delay, chanel time(most 0.25), 0 or 1 does not need minion check 0
        if CastPos ~= nil then
