BuffLib = {
    ...
    Items = {
        ...
        Sheen = function(self, Hero)
            local buff = Hero.BuffData:GetBuff("sheen")
            return buff and buff.Count_Alt > 0
        end,
    },
    Caitlyn = {
    ...
    Gangplank = {
        TrialByFire = function(self, Hero)
            local buff = Hero.BuffData:GetBuff("gangplankpassiveattack")
            return  buff and buff.Count_Alt > 0
        end,
        BarrelStartTime = function(self, Barrel)
            local buff = Barrel.BuffData:GetBuff("gangplankebarrellife")
            if buff then
    ...
    Ezreal = {
        W = function(self, Hero)
            local buff = Hero.BuffData:GetBuff("ezrealwattach")
            return buff.Valid
        end
    }
}
