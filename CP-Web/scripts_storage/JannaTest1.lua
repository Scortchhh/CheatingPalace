...
local function GetDist(source, target) --- GetDist needs to be the same all the way throughout, if you do, function Annine:GetDist then self:GetDist will also work or Annie:GetDist
    return math.sqrt((target.x - source.x) ^ 2 + (target.z - source.z) ^ 2)
end

function Janna:Q()
    if self.UseQCombo.Value == 1 and Engine:SpellReady("HK_SPELL1") then
    local CastPos = Prediction:GetCastPos(myHero.Position, self.QRange - 50, 666.67, 240, 0.25, 0) -- positionHero, range of spell - 50, speed of missle, width, delay, chanel time(most 0.25), 0 or 1 does not need minion check 0
    if CastPos ~= nil then
...
function Janna:EAlly()
	if self.ComboUseE.Value == 1 and Engine:SpellReady("HK_SPELL3") then
        local AttackingFriend = Janna:ETarget(self.ERange)
        local HeroList = ObjectManager.HeroList
        local FCount = Janna:EAllyCount(200)
        --print(FCount)
        for k, Ally in pairs(HeroList) do
            if Ally.Team == myHero.Team and not Ally.IsDead and Ally.IsTargetable then
                ...
                if TickOnAlly == 1 then
                    if Janna:GetDist(Ally.Position, myHero.Position) < self.ERange -15 then
