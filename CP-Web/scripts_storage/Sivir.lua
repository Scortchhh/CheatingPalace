...
function Sivir:__init()
    ...
    self.UseWCombo = self.SivirCombo:AddCheckbox("Use W in combo", 1)
...
function Sivir:Combo() 
    ...
    if target then
        if Engine:SpellReady("HK_SPELL1") and self.UseQCombo.Value == 1 and Orbwalker.Attack == 0 then
    ...
    if target then
        if Engine:SpellReady('HK_SPELL2') and Orbwalker.ResetReady == 1 and self.UseWCombo.Value == 1 then
...
function Sivir:Harass() 
    ...
    if target then
        if Engine:SpellReady("HK_SPELL1") and self.UseQHarass.Value == 1 and Orbwalker.Attack == 0 then
            if Sivir:GetDistance(myHero.Position, target.Position) <= 1100 then
                local castPos = Prediction:GetCastPos(myHero.Position, self.QRange, self.QSpeed, self.QWidth, self.QDelay, 0, true, self.QHitChance, 1)
                if castPos ~= nil then
                    Engine:CastSpell("HK_SPELL1", castPos, 1)
...
function Sivir:UseW()
    if Engine:SpellReady("HK_SPELL2") and self.UseWWaveClear.Value == 1 and Orbwalker.Attack == 0 then
        ...
        for I, Missile in pairs(Missiles) do
            if Missile.Team ~= myHero.Team then 
                ...
                if Info ~= nil and Info.Type == 0 then
                    if Sivir:CheckCollision(Missile.MissileStartPos, Missile.MissileEndPos, Info.Radius+myHero.CharData.BoundingRadius + 25) and self:GetDistance(Missile.Position, myHero.Position) < 400 then
