-- made by MrWong, credits to scortch and Christoph ivern updated 1.2
Ivern = {
    rRange = 450,
    eRange = 750,
    qRange = 1100,
	...
	for I,Hero in pairs(HeroList) do	
		if Hero.Team == myHero.Team and Hero.IsTargetable then
			if GetDistance(myHero.Position, Hero.Position) < self.eRange then
				local EnemyCount = EnemiesInRange(Hero.Position, 400)
	...
	for I,Minion in pairs(MinionList) do	
		if Minion.Team == myHero.Team and Minion.Name == "IvernMinion" then
			if GetDistance(myHero.Position, Minion.Position) < self.eRange then
				local EnemyCount = EnemiesInRange(Minion.Position, 350)
				if EnemyCount > 0 then
					return Minion
				end
			end
...
function Ivern:Combo()
    ...
    if target then
        if self.ComboUseR.Value == 1 and Engine:SpellReady("HK_SPELL4") then
            if not myHero.BuffData:GetBuff("IvernRRecast").Valid then
            ...
            if self.ComboUseQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
                ...
                if PredPos ~= nil then
                    if GetDistance(StartPos, PredPos) <= self.qRange then
            ...
            --[[if self.ComboUseW.Value == 1 and Engine:SpellReady("HK_SPELL2") then
                local Passive = myHero.BuffData:GetBuff("ivernwpassive")
                if GameClock.Time + 2 >= Passive.EndTime then
                    if GetDistance(myHero.Position, target.Position) <= myHero.AttackRange then
