...
function Corki:KillSteal()
    if self.ComboRKS.Value == 1 and Engine:SpellReady("HK_SPELL4") and myHero:GetSpellSlot(3).Charges > 0 then
        local target = Orbwalker:GetTarget("Combo", 1400)
        if target ~= nil then
            if myHero.BuffData:GetBuff("mbcheck2").Valid then
                if self:KillStealR(target) then
                    local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.BigRRange, self.RSpeed, self.RWidth, self.RDelay, 1, true, self.RHitChance, 1)
                    if PredPos ~= nil then
    ...
    if self.ComboQKS.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        ...
        if target ~= nil then
            if self:KillStealQ(target) then
                local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.QRange + 100, self.QSpeed, self.QWidth, self.QDelay, 0, true, self.QHitChance, 0)
                if PredPos ~= nil then
...
function Corki:Combo()
    ...
    if self.ComboR.Value == 1 and Engine:SpellReady("HK_SPELL4") and myHero:GetSpellSlot(3).Charges > 0 then
        local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.RRange, self.RSpeed, self.RWidth, self.RDelay, 1, true, self.RHitChance, 1)
        if PredPos ~= nil then
            Engine:CastSpell("HK_SPELL4", PredPos, 1)
            return
        end
...
function Corki:Harass()
    if self.HarassR.Value == 1 and Engine:SpellReady("HK_SPELL4") then
        ...
        local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.RRange, self.RSpeed, self.RWidth, self.RDelay, 1, true, 0.35, 1)
        if PredPos ~= nil then
            Engine:CastSpell("HK_SPELL4", PredPos, 1)
            return
        end
    ...
    if self.HarassQ.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        ...
        local PredPos, Target = Prediction:GetCastPos(myHero.Position, self.QRange + 100, self.QSpeed, self.QWidth, self.QDelay, 0, true, 0.35, 0)
        if PredPos ~= nil then
