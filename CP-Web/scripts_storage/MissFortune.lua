...
function MF:__init()
    ...
    self.UseQComboKillable = self.MFCombo:AddCheckbox("^only if minion killable with Q", 1)
    ...
    self.UseRSlider = self.MFCombo:AddSlider("X amounts of targets", 1, 1, 5, 1)
    self.UseRSliderOnly = self.MFCombo:AddCheckbox("^only use this condition and not killable", 0)
...
function MF:SaveSettings()
    ...
    SettingsManager:AddSettingsInt("UseQComboKillable", self.UseQComboKillable.Value)
    ...
    SettingsManager:AddSettingsInt("UseRSlider", self.UseRSlider.Value)
    SettingsManager:AddSettingsInt("UseRSliderOnly", self.UseRSliderOnly.Value)
    -------------------------------------------
...
function MF:LoadSettings()
    ...
    self.UseQComboKillable.Value = SettingsManager:GetSettingsInt("Combo", "UseQComboKillable")
    ...
    self.UseRSlider.Value = SettingsManager:GetSettingsInt("Combo", "UseRSlider")
    self.UseRSliderOnly.Value = SettingsManager:GetSettingsInt("Combo", "UseRSliderOnly")
    -------------------------------------------
    ...
    for i, Minion in pairs(MinionList) do
        if Minion.Team ~= myHero.Team and Minion.IsTargetable and Minion.IsDead == false then
            if MF:GetDistance(myHero.Position, Minion.Position) <= 1400 then
                local castPos = Prediction:GetCastPos(Minion.Position, 300, self.QSpeed, self.QWidth, self.QDelay, 0, true, self.QHitChance, 1)
                if castPos ~= nil then
...
function MF:SetOrbActive()
    if not myHero.BuffData:GetBuff("missfortunebulletsound").Valid and GameClock.Time > self.MfUltTimer then
        self.IsUlting = false
        Orbwalker.Enabled = 1
    end
end
...
function MF:Combo()
    if Engine:SpellReady("HK_SPELL4") and self.UseRCombo.Value == 1 and not self.IsUlting then
        local HeroList = ObjectManager.HeroList
        for i, Hero in pairs(HeroList) do 
            if Hero.Team ~= myHero.Team and Hero.IsDead == false and Hero.IsTargetable then
                if MF:GetDistance(myHero.Position, Hero.Position) <= 1300 and MF:GetDistance(myHero.Position, Hero.Position) > MF:GetAttackRange() - 100 then
                    local waves = 0
                    ...
                    if Hero.Health <= rDmg then
                        if self.UseRSliderOnly.Value == 1 then
                            local EnemiesAround = self:EnemiesInRange(Hero.Position, 400)
                            if EnemiesAround >= self.UseRSlider.Value then
                                local castPos = Prediction:GetCastPos(myHero.Position, self.RRange, self.RSpeed, self.RWidth, self.RDelay, 0, 0, self.RHitChance, 1)
                                if castPos ~= nil then
                                    self.IsUlting = true
                                    self.MfUltTimer = GameClock.Time + 0.5
    ...
    if self.IsUlting == false then
        ...
        if target then
            ...
            if qTarget ~= nil then
                if Engine:SpellReady("HK_SPELL1") and self.UseQCombo.Value == 1 then
                    if MF:GetDistance(myHero.Position, qTarget.Position) <= MF:GetAttackRange() then
                        if self.UseQComboKillable.Value == 1 then
                            local qDmg = 0 + (20 * myHero:GetSpellSlot(0).Level) + (myHero.BaseAttack + myHero.BonusAttack) + (myHero.AbilityPower * 0.35)
                            qDmg = self:GetDamage(qDmg, true, qTarget)
                            if qTarget.Health <= qDmg then
            ...
            if Engine:SpellReady("HK_SPELL3") and self.UseECombo.Value == 1 then
                if MF:GetDistance(myHero.Position, target.Position) <= 1100 then
...
function MF:OnTick()
    if GameHud.Minimized == false and GameHud.ChatOpen == false then
        ...
        if self.MfUltTimer ~= nil then
            if GameClock.Time > self.MfUltTimer then
                self:SetOrbActive()
            end
        end
