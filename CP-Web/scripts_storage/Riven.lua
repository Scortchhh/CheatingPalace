...
function Riven:PassiveRation()
    if GetMyLevel() > 1 and GetMyLevel() < 6 then
        return 0.3
    end
    if GetMyLevel() > 5 and GetMyLevel() < 9 then
        return 0.36
    end
    if GetMyLevel() > 8 and GetMyLevel() < 12 then
        return 0.42
    end
    if GetMyLevel() > 11 and GetMyLevel() < 15 then
        return 0.48
    end
    if GetMyLevel() > 14 and GetMyLevel() < 18 then
        return 0.54
    end
    if GetMyLevel() > 17 then
        return 0.6
    end
    return 0
end

function Riven:AAandPassive(Target)
    local TotalAD = myHero.BonusAttack + myHero.BaseAttack
    local AADmg = GetDamage(TotalAD * 3, true, Target)
    local WDmg = GetDamage(25 + (30 * myHero:GetSpellSlot(1).Level) + myHero.BonusAttack * 1, true, Target)
    ...
    local Passive = GetDamage(self:PassiveRation() * TotalAD * 3, true, Target)
    local FinalFullDmg = AADmg + RDmg + WDmg
...
function Riven:Combo()
    ...
    if self.ComboR.Value == 1 and Engine:SpellReady("HK_SPELL4")  then
        ...
        if target ~= nil then
            if self:AAandPassive(target) then
                Engine:ReleaseSpell("HK_SPELL4", target.position)
            end
        end
    end    
    ...
    if Engine:SpellReady('HK_SPELL3') and self.ComboE.Value == 1 then
        ...
        if GetDist(myHero.Position, target.Position) >= self.ERange then
            Engine:ReleaseSpell("HK_SPELL3", target.Position)
        end
    end
    ...
    if Engine:SpellReady("HK_SPELL1") and self.ComboQ.Value == 1 then
        ...
        if target then
            if GetDist(myHero.Position, target.Position) < getAttackRange() and Orbwalker.ResetReady == 1 then
                Engine:ReleaseSpell("HK_SPELL1", target.Position)
            end
        end
    end
