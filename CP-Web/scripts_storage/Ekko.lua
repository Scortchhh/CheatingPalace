...
function Ekko:__init() 
    ...
    self.ScriptVersion = "         dEkko Ver: 1.11 CREDITS Derang3d" 
...
function Ekko:Ultimate()
    if self.ChronoShadow then
        local rPos = self.ChronoShadow.Position
        if self.ComboR.Value == 1 and Engine:SpellReady('HK_SPELL4') then
            ...
            for i, target in pairs(HeroList) do
                if target.Team ~= myHero.Team and target.IsDead == false then
                    if rPos and GetDist(rPos, target.Position) <= 325 then
                        local EkkoR = GetDamage((150 * myHero:GetSpellSlot(3).Level) + (myHero.AbilityPower * 1.5), false, target)
                        if EkkoR >= target.Health and Orbwalker:IsImmune(target) == false then
...
function Ekko:OnTick()
    ...
    if GameHud.Minimized == false and GameHud.ChatOpen == false then
        self.ChronoShadow = Ekko:GetChronoShadow()
        Ekko:Ultimate()
...
function Ekko:OnDraw()
    ...
    if Engine:SpellReady("HK_SPELL4") and self.DrawR.Value == 1 then
        if self.ChronoShadow then
            Render:DrawCircle(self.ChronoShadow.Position, self.RRange ,100,150,255,255) -- values Red, Green, Blue, Alpha(opacity)      
        end
    end
end
