...
<div class="container">
    ...
    <div class="col-md mt-2">
        <form method="post">
            ...
            <h3 class="mb-3">Manage subscription time</h3>
            <br>
        ...
        <div class="col-md mt-2">
            <form method="post" enctype='multipart/form-data'>
                ...
                <h3>Type champion name with capital starting letter</h3>
                    ...
                    <textarea name="description" class="form-control" aria-label="With textarea" placeholder="[+] Updated Q swap logic&#10;[+] Updated W range checks&#10;[+] Updated E slow checks&#10;[+] Updated R dmg calcs"></textarea>
                    <input style="border: 0.2px solid black" class="mt-2" type='file' name='file[]' multiple />
    ...
    if ($succes == $countfiles) {
        ...
        if ($category != "Vip") {
            ...
            if ($countVIP <= 0) {
                ...
                $statement2 = $pdo->query("INSERT INTO scripts_vip_key(scriptname, encryption_key, version, author) VALUES('$script', '$encryptionKey', '1', '$username')");
        ...
        if(empty($description)) {
            $description = "<@&621399169499922442> \nUpdated $script\n [+] General Improvements";
        } else {
            $description = "<@&621399169499922442> \nUpdated $script\n" . $description;
        }
        $json_data = array ('content'=>"$description");
...
if (isset($_POST['changeSubTime'])) {
    try{
        $user = $_POST['user'];
        echo '<script>location.replace("update_subtime.php?hwid='.$user.'");</script>';
    }catch(Exception $e){
        echo $e;
    }
}
